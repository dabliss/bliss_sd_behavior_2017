...
if __name__ == '__main__':
    ...
    if isinstance(sub_num, int):
        ...
        diff_rad = np.load(f_name + '_d_stim_perception.npy')
        resid_error_rad = np.load(f_name +
                                  '_global_resid_error_perception.npy')
    else:
        diff_rad = np.array([])
        ...
        for n in sub_num:
            ...
            diff_rad = np.concatenate([diff_rad,
                                       np.load(f_name +
                                               '_d_stim_perception.npy')])
            resid_error_rad = np.concatenate(
                [resid_error_rad,
                 np.load(f_name + '_global_resid_error_perception.npy')])
