...
if __name__ == '__main__':
    ...
    if isinstance(sub_num, int):
        ...
        diff_rad = np.load(f_name + '_d_stim_all_delays.npy')
        resid_error_rad = np.load(f_name +
                                  '_global_resid_error_all_delays.npy')
    else:
        diff_rad = np.array([])
        ...
        for n in sub_num:
            ...
            diff_rad = np.concatenate([diff_rad,
                                       np.load(f_name +
                                               '_d_stim_all_delays.npy')])
            resid_error_rad = np.concatenate(
                [resid_error_rad,
                 np.load(f_name + '_global_resid_error_all_delays.npy')])
    ...
    for i_perm in range(n_permutations):
        ...
        params[i_perm, :] = ba.fit_clifford(resid_error_rad[ind], diff_rad[ind])
    ...
    f_name = os.path.join(results_dir,
                          'bootstrap_clifford_all_delays_s%s.txt' % sub_name)
